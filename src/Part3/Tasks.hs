...
mostFreq :: [Int] -> Int
mostFreq numbers = read [head $ maximumBy (comparing length) digits]
    where
        digits = group $ sort $ concatMap show [abs x | x <- numbers]

-- Дан список lst. Вернуть список элементов из lst без повторений, порядок может быть произвольным.
uniq :: (Eq a) => [a] -> [a]
uniq [] = []
uniq (h : t) | h `elem` t = uniq t
             | otherwise = h : uniq t
...
grokBy f l = [(key, [x | x <- l, f x == key]) | key <- uniq (map f l)]
